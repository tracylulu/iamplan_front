    ...
    <el-row class="title">
      <el-col :span="24">
        <div class="navBar">
          ...
          <div class="navItem navItemSplit">&gt;</div>
          <div class="navItem navItemCol">二级部门主管审核</div>
        </div>
      </el-col>
    </el-row>  
    <div class="formBox">
      <el-row type="flex" justify="space-between">
        <h3 class="titleHeaderText">{{applymonth}}  二级部门主管审核</h3>
      </el-row>
...
export default {
  ...
  data() {
    return {
      tableItemSelectFlag: false,// 列表条目是否可选
      tableHead: [],
      tableData: [],
      applymonth: '',
      assetStage: '4',
  ...
  methods: {
    ...
    getBudgeInfoList() {
      let that = this;
      that.$axios.get(`${urlConfig.assetPlanDept3Mag.GETBUDGETINFOLIST}?applymonth=${that.applymonth}&applyuser=${that.reviewer}&apstage=4`).then(function (res) {
        if (res.data.success) {
          if(res.data.data) {
    ...
    getDept2InfoList() {
      let that = this;
      this.$store.dispatch('getDept2InfoList', {
        apstage: 4,
        dept2Manager: that.reviewer,
          ...
          if(res.data && res.data.length > 0) {
            ...
            that.tableData.forEach((groupItem, groupIndex) => {
              groupItem.DataSet.forEach((item, index) => {
                ...
                if(!item.dept2checknote) {
                  item.dept2checknote = '同意'
                }
              })
            })
            that.calcActualMoney()
    ...
    // 二级部门审核页面提交至计划员审核
    submitInfoFromDept2ToPlanner() {
      let that = this;
      this.$store.dispatch('submitInfoFromDept2ToPlanner', {
    ...
    updateReviewInfo(type) {
      ...
      that.tableData.forEach((groupItem, groupIndex) => {
        groupItem.DataSet.forEach((item, index) => {
          ...
          params.dept2checknote.push(item.dept2checknote)
        })
      })
      this.$store.dispatch('updateDept2InfoList', params).then(res => {
        if(res.success) {
          if(type == 'save') {
            that.$message({
              message: res.message,
            ...
            that.submitInfoFromDept2ToPlanner()
          }
        } else {
          that.$message({
            message: res.message,
    ...
    calcActualMoney() {
      ...
      this.budgeInfoList.forEach((budgetItem, budgetIndex) => {
        let deptCodeType = ''
        if(budgetItem.deptlevel == 2) { // 二级部门
          deptCodeType = 'dept2code'
        } else if (budgetItem.deptlevel == 3) {
          deptCodeType = 'deptcode'
        }
        that.tableData.forEach((groupItem, groupIndex) => {
          if(groupItem.groupId == 2) {
            let totalMoney = 0
            groupItem.DataSet.forEach((item, index) => {
              if(budgetItem.deptCode == item[deptCodeType] && item.expensetype == 2) {
